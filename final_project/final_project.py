...
def train_model(train_loader, test_loader, config, dataset_name, verbose=True):
...
def main(use_x1=False, use_x2=False, use_x3=False, use_all=False):
    ...
    for lr, hs, nl in tqdm(param_space):
        ...
        if use_x1:
            loss_x1 = train_model(X1_train_loader, X1_test_loader, param_config, "X1", verbose=True)
        if use_x2:
            loss_x2 = train_model(X2_train_loader, X2_test_loader, param_config, "X2", verbose=True)
        if use_x3:
            loss_x3 = train_model(X3_train_loader, X3_test_loader, param_config, "X3", verbose=True)
        if use_all:
            loss_all = train_model(X1X2X3_train_loader, X1X2X3_test_loader, param_config, "X1X2X3", verbose=True)
...
if __name__ == '__main__':
    ...
    main(args.X1, args.X2, args.X3, args.X1X2X3)
